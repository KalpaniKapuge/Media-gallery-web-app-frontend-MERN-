...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await api.get('/media'); // returns { success, data, ... } or array
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="p-4 bg-white rounded shadow">
          ...
          <p className="mt-2 text-lg font-semibold">{user ? `Hello, ${user.name}` : 'Hello, Guest'}</p>
          <p className="text-sm text-gray-600 mt-2">{user ? user.email : 'Please login to manage your media.'}</p>
        </div>
